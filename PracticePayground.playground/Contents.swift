...
class Planet
{
    ...
    convenience init() // convinience initializer
    {
        self.init(name:"[No planets]") // ultimatly a init is called that will initiallize all fields
    }
}

var mercuryPlanet =  Planet(name: "Mercury")//designated initializer
var noPlanet = Planet() //convinience initializer
print("\(mercuryPlanet.name)")
print("\(noPlanet.name)")
...
var somePlanets = Planets(name: "Mars")
print("\(somePlanets.name) : count \(somePlanets.count)")
...
//Here, by using forced unwrapping , since candidate is nil , an exception is thrown
//let cand = ElectionPoll()
//if let candname:String? = cand.candidate!.name{
//    print("candname = \(candname)")
//}
//else
//{
...
//Soultion  : optional chaining : gives an solution

let cand = ElectionPoll()
if let candname:String? = cand.candidate?.name{ // deos not throw an exception
    print("candname = \(candname)")
}
else
{
